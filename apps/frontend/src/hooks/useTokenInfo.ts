...
import { TokenInfo, getTokenInfo } from "../api/token-api"
...
export function useTokenInfo(tokenId = "") {
  ...
  useEffect(() => {
    if (isFetched || $tokenInfoCache.get()[tokenId] === "loading" || tokenId === nativeTokenInfo.id)
      return
  ...
  if (tokenId === nativeTokenInfo.id) return nativeTokenInfo
